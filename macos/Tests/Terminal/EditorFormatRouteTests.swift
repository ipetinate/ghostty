...
/// in a repository that declares no formatter answered ⇧⌘F by naming
/// `marksman` — the wrong tool, for a file the project formats from the
/// command line.
struct EditorFormatRouteTests {
    private func route(
        ...
        EditorFormatRoute.usesFormatter(
            adoption: .unadopted,
            trigger: trigger,
            server: server,
    ...
    // MARK: What the project declared wins

    /// The whole reason the three keys exist. A project that declares the tool
    /// has already decided how its files are written, and a language server
    /// formatting them another way is the wrong answer arriving faster — so
    /// nothing defers, not even a save.
    @Test func aToolTheProjectDeclaredBeatsEverything() {
        for trigger in [EditorFormatTrigger.command, .save] {
            for server in [LSPServerStatus.running, .starting, .notInstalled, nil] {
                #expect(
                    EditorFormatRoute.usesFormatter(
                        adoption: .adopted,
                        trigger: trigger,
                        server: server,
                        serverFormats: true),
                    "\(trigger) \(String(describing: server))")
            }
        }
    }

    // MARK: When a tool the project never declared gets the last word

    @Test func markdownWhoseServerDoesNotFormatGoesToTheTool() {
        #expect(route())
    }

    /// A language with no server at all — nothing else was ever going to
    /// answer.
    @Test func aFileWithNoServerConfiguredGoesToTheTool() {
        #expect(route(server: nil))
    }

    /// A sentence about an uninstalled `marksman` is not what somebody asking
    /// to format Markdown needs.
    @Test func aBrokenServerDoesNotBlockTheTool() {
    ...
    /// not a preference — a TypeScript project that declares no formatter
    /// still gets `tsserver`.
    ...
    /// a TypeScript file would format it with the tool's defaults instead.
    ...
    /// The rule reached through the dispatcher is the same rule, so a tool
    /// that asks nothing of a project keeps the behaviour it had.
    @Test func aToolThatDeclaresNothingTakesTheDeferringRoute() {
        for trigger in [EditorFormatTrigger.command, .save] {
            #expect(
                EditorFormatRoute.usesFormatter(
                    adoption: .undeclared, trigger: trigger,
    ...
    /// The difference from the unadopted route, and the deliberate one: a
    ...
    @Test func aHandshakeThatNeverFinishesStopsBlockingTheTool() {
