...
/// Which of a formatter's outcomes are allowed to interrupt the reader.
///
...
/// missing-tool dialog on a keystroke that only asked for the file to be
/// written.
struct FormatNoticeTests {
    ...
    /// No tool owns this file, so nothing has been decided yet: the
    ...
    @Test func aReformatIsSilentOnEitherTrigger() {
        let edit = FormatEdit(range: NSRange(location: 0, length: 1), newText: "  ")
    ...
    @Test func aFailureOnTheExplicitCommandCarriesTheReason() {
        let reason = ExternalFormatterFailure
            .failed(tool: "Tool", status: 2, message: "[error] main.ts: Unexpected token (3:7)")
            .reason
    ...
    /// project with no tool installed the alternative is this dialog on
    ...
    @Test func aFailureOnSaveIsSilent() {
        let missing = ExternalFormatterFailure.notFound(tool: "Tool", hint: "brew install tool")
        let notice = FormatAttempt.failed(missing.reason).notice(for: .save)
